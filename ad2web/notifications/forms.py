...
class CreateNotificationForm(Form):
    type = SelectField('Notification Type', choices=[nt for t, nt in list(NOTIFICATIONS.items())])
...
class EditNotificationMessageForm(Form):
    id = HiddenField()
    text = TextAreaField('Message Text', [InputRequired(), Length(max=255)])
...
class EditNotificationForm(Form):
    type = HiddenField()
    description = StringField('Description', [InputRequired(), Length(max=255)], description='Brief description of this notification')
    ...
    subscriptions = MultiCheckboxField('Notification Events', choices=[(str(k), v) for k, v in list(SUBSCRIPTIONS.items())])
    ...
    def populate_from_settings(self, id):
        ...
        if subscriptions:
            self.subscriptions.data = [k if v == True else False for k, v in list(json.loads(subscriptions).items())]
...
class EmailNotificationInternalForm(Form):
    source = StringField('Source Address (From)', [InputRequired(), Length(max=255)], default='youremail@example.com', description='Emails will originate from this address')
    destination = StringField('Destination Address (To)', [InputRequired(), Length(max=255)], description='Emails will be sent to this address')

    subject = StringField('Email Subject', [InputRequired(), Length(max=255)], default='AlarmDecoder: Alarm Event', description='Emails will contain this text as the subject')

    server = StringField('Email Server (Configured using local server by default, not preferred due to ISP filtering)', [InputRequired(), Length(max=255)], default='localhost')
    port = IntegerField('Server Port (If using your own server, check that port is not filtered by ISP)', [InputRequired(), NumberRange(1, 65535)], default=25)
...
class PushoverNotificationInternalForm(Form):
    token = StringField('API Token', [InputRequired(), Length(max=30)], description='Your Application\'s API Token')
    user_key = StringField('User/Group Key', [InputRequired(), Length(max=30)], description='Your user or group key')
...
class TwilioNotificationInternalForm(Form):
    account_sid = StringField('Account SID', [InputRequired(), Length(max=50)], description='Your Twilio Account SID')
    auth_token = StringField('Auth Token', [InputRequired(), Length(max=50)], description='Your Twilio User Auth Token')
    number_to = StringField('To', [InputRequired(), Length(max=15)], description='Number to send SMS/call to')
    number_from = StringField('From', [InputRequired(), Length(max=15)], description='Must Be A Valid Twilio Phone Number')
    class Meta:
        csrf = False
    def __init__(self, *args, **kwargs):
        kwargs['csrf_enabled'] = False
...
class TwiMLNotificationInternalForm(Form):
    ...
    number_to = StringField('To', [InputRequired(), Length(max=15)], description='Number to send SMS/call to')
    number_from = StringField('From', [InputRequired(), Length(max=15)], description='Must Be A Valid Twilio Phone Number')
    twimlet_url = StringField('Twimlet URL', [InputRequired()], default="http://twimlets.com/message", description='Your twimlet URL (http://twimlets.com/message)')
    class Meta:
        csrf = False
    def __init__(self, *args, **kwargs):
        kwargs['csrf_enabled'] = False
...
class ProwlNotificationInternalForm(Form):
    prowl_api_key = StringField('API Key', [InputRequired(), Length(max=50)], description='Your Prowl API Key')
    prowl_app_name = StringField('Application Name', [InputRequired(), Length(max=256)], description='Application Name to Show in Notifications', default='AlarmDecoder')
...
class GrowlNotificationInternalForm(Form):
    growl_hostname = StringField('Hostname', [InputRequired(), Length(max=255)], description='Growl server to send notification to')
    growl_port = StringField('Port', [InputRequired(), Length(max=10)], description='Growl server port', default=23053)
    growl_password = PasswordField('Password', description='The password for the growl server')
    growl_title = StringField('Title', [InputRequired(), Length(max=255)], description='Notification Title', default=GROWL_TITLE)
...
class CustomPostInternalForm(Form):
    custom_url = StringField('URL', [InputRequired(), Length(max=255)], description='URL to send data to (ex: www.alarmdecoder.com)')
    custom_path = StringField('Path', [InputRequired(), Length(max=400)], description='Path to send variables to (ex: /publicapi/add)')
...
class MatrixNotificationInternalForm(Form):
    domain = StringField('Domain', [InputRequired(), Length(max=255)], description='Domain or IP of matrix server ex. matrix.org')
    room_id = StringField('Room ID', [InputRequired(), Length(max=300)], description='Room ID and domain ex. !DPNBnAVwxPMvNKTvvY:matrix.org')
    token = StringField('Token', [InputRequired(), Length(max=300)], description='The long device authentication token. ex. D0gMQowMDI.....')
    custom_values = FieldList(FormField(CustomValueForm), validators=[Optional()], label=None)
    add_field = ButtonField('Add Field', onclick='addField();')
    class Meta:
        csrf = False
