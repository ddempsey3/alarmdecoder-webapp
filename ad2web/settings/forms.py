...
class ProfileForm(Form):
    ...
    sex_code = RadioField("Sex", [AnyOf([str(val) for val in list(SEX_TYPE.keys())])], choices=[(str(val), label) for val, label in list(SEX_TYPE.items())])
...
class EmailConfigureForm(Form):
    ...
    port = IntegerField('Server Port', [InputRequired(), NumberRange(1, 65535)], description='ex: 25 for normal or 587 for TLS')
    tls = BooleanField('Use TLS?', default=False)
    auth_required = BooleanField('Authentication InputRequired?',default=False)
    ...
    default_sender = StringField('From Email', [InputRequired(), Length(max=255)], default='root@alarmdecoder', description='Emails will come from this address')
    submit = SubmitField('Save')

class UPNPForm(Form):
    internal_port = IntegerField('Internal Port', [InputRequired()], default=443, description='Internal Port to Forward To')
    external_port = IntegerField('External Port', [InputRequired()], default=random.randint(1200,60000), description='External Port to map to Internal Port')

    submit = SubmitField('Save')

class VersionCheckerForm(Form):
    version_checker_timeout = IntegerField('Timeout in Seconds', [InputRequired(), NumberRange(600)], default=600, description='How often to check for version updates')
...
class ExportConfigureForm(Form):
    ...
    email_address = StringField('Email Address', [Optional(), Length(max=255)], description='Email Address to Send Export to')
    local_file = BooleanField('Save to Local File?', default=True)
    local_file_path = StringField('Path to Save file', [Optional(), Length(max=255)], default=os.path.join(INSTANCE_FOLDER_PATH, 'exports'), description='Path on AlarmDecoder to Save Export')
